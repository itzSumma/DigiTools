    ...
    <section className="mx-auto w-[min(1120px,calc(100%-1.5rem))] py-16 md:py-20" id="steps">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {steps.map((step, index) => (
          <article key={step.title} className="card border border-base-300 bg-base-100 shadow-sm">
            <div className="card-body items-start p-8">
              <span className="badge badge-primary badge-lg rounded-full">0{index + 1}</span>
              <div className="mt-2 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10">
                <img src={step.icon} alt="" aria-hidden="true" className="h-8 w-8 object-contain" />
              </div>
